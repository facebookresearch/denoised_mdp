...
from denoised_mdp.agents import DenoisedMDP, DynamicsBackpropagateActorCritic, SoftActorCritic, BasePolicyLearning, VAE
...
class Config:
    ...
    class LearningConfig:
        ...
        model_learning: VAE.Config = VAE.Config()
        ...
        class ExplorationConfig:
            ...
            total_steps: int = attrs.field(default=int(5e6), validator=attrs.validators.gt(0))
...
class InstantiatedConfig(Config):
    ...
    # start ckpting (incl. replay buffer), and finally kill the training job.
    #
    ...
    def __attrs_post_init__(self):
        ...
        if os.path.isdir(self.output_dir) and not self.overwrite_output:
            raise RuntimeError("Output directory {} exists, exiting.".format(self.output_dir))
        ...
        if os.path.exists(self.job_complete_file) and not self.overwrite_output:
            print("Complete run found at {}, exiting.".format(self.output_dir))
            sys.exit(0)
    ...
    class LearningConfig(Config.LearningConfig):
        ...
        model_learning: VAE.Config.InstantiatedT
