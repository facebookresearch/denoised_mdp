...
@SUITE.add('benchmarking')
def walk(time_limit=_DEFAULT_TIME_LIMIT, random=None, environment_kwargs=None, noisy_sensor=False):
...
class Physics(common.NoisyMuJoCoPhysics):
  ...
  def torso_upright(self):
    ...
    proj = self.named.data.xmat['torso', 'zz']
    return proj

  def torso_height(self):
    """Returns the height of the torso."""
    torso_height = self.named.data.xpos['torso', 'z']
    if self.noise is not None:
      torso_height = torso_height + self.noise[0] * _STAND_HEIGHT
    return torso_height

  def horizontal_velocity(self):
    """Returns the horizontal velocity of the center-of-mass."""
    horizontal_velocity = self.named.data.sensordata['torso_subtreelinvel'][0]
