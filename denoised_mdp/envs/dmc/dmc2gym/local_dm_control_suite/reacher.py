...
def easy(time_limit=_DEFAULT_TIME_LIMIT, random=None, environment_kwargs=None, noisy_sensor=False):
...
class Physics(common.NoisyMuJoCoPhysics):
  ...
  def finger_to_target_dist(self, *, success_radius):
    """Returns the signed distance between the finger and target surface."""
    dist = np.linalg.norm(self.finger_to_target())
    if self.noise is not None:
      dist = dist + self.noise[0] * success_radius
    return dist
...
class Reacher(base.Task):
  ...
  def get_reward(self, physics):
    radius = physics.named.model.geom_size[['target', 'finger'], 0].sum()
    return rewards.tolerance(physics.finger_to_target_dist(maybe_noisy=True, success_radius=radius), (0, radius))
